...
/*
  ...
  Invariante de Representaci車n:
  ----------------------------
  Si !r.laraiz.null(),
  -   r.laraiz.parent().null().

  Para cualquier nodo n del 芍rbol:
  Si !n.left().null()
      n.left().parent() == n;
  Si !n.right().null()
      n.right().parent() == n;
				...
				typename bintree<T>::node n)
{
  if (&a != this) {
    ...
    if (!laraiz.null())
      laraiz.parent(typename bintree<T>::node());
  } else {  // Reemplazar el receptor por un sub嫫bol suyo.
    if (laraiz != n) {
